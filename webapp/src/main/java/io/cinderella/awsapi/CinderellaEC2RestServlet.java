...
public class CinderellaEC2RestServlet extends HttpServlet {

   private static final long serialVersionUID = -6168996266762804888L;

   public static final Logger logger = Logger.getLogger(CinderellaEC2Servlet.class);
...
//      File propertiesFile = ConfigurationHelper.findConfigurationFile("ec2-service.properties");
      String configLocation = "/WEB-INF/conf/ec2-service.properties";
      InputStream propertiesFile = config.getServletContext().getResourceAsStream(configLocation);
      Properties EC2Prop = null;

      if (null != propertiesFile) {
         logger.info("Use EC2 properties file: " + configLocation);
         EC2Prop = new Properties();
         try {
            EC2Prop.load(propertiesFile);
         } catch (FileNotFoundException e) {
            logger.warn("Unable to open properties file: " + propertiesFile, e);
         } catch (IOException e) {
            logger.warn("Unable to read properties file: " + propertiesFile, e);
         }
